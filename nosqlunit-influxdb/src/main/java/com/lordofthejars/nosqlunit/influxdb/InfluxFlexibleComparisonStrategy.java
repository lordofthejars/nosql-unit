...
/**
 * Comparison strategy that checks that all the expected data exists in the Influx
 ...
 * Influx. It just assure that the expected data exists.
 * <p>
 * Checks the following assertions:
 * <li>
 * <ul>
 * Checks that all the expected measurements are present in Influx DB, but accepts other
 * measurements stored in the database that are not defined in the expected file.
 * </ul>
 * <ul>
 * Checks that all the expected objects are present in Influx DB, but accepts other
 * objects stored in the same measurements that are not defined as expected.
 * </ul>
 * <ul>
 ...
 * <li>
 * <ol>
 * measurement.property : When are defined both measurement and property name the
 * exclusion will only affect to the indicated measurement. e.g:
 * With @IgnorePropertyValue(properties = {"book.date"}), the property date will be
 * ignored in each object of the 'book' measurement. If other objects in different
 * measurements have the property 'date' it won't be ignored.
 * </ol>
 * <ol>
 * property : When only is defined the property name it will be excluded from all objects
 * in any expected measurement. e.g: With @IgnorePropertyValue(properties = {"date"}), the
 * property 'date' will be ignored in each object, no matter the measurement.
 * </ol>
 * </li>
 * </p>
 *
 * The values of the properties to be ignored should be named following the rules for
 * valid measurement and property names defined in <a href=
 * "https://docs.aws.amazon.com/amazoninfluxdb/latest/developerguide/HowItWorks.NamingRulesDataTypes.html">
 ...
 */
public class InfluxFlexibleComparisonStrategy implements InfluxComparisonStrategy {
    ...
    public boolean compare(final InfluxDbConnectionCallback connection, final InputStream dataset) throws IOException {
        ...
        final Map<String, List<ExpectedPoint>> parsedData = objectMapper.readValue(dataset, ExpectedDataSet.TYPE_REFERENCE);
        parsedData.entrySet() //
                .stream() //
        ...
        InfluxDbAssertion.flexibleAssertEquals(new ExpectedDataSet(parsedData), ignorePropertyValues,
                connection.dbClient());
